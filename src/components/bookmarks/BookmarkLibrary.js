...
import { useInfiniteBookmarks } from '../../hooks';
import BookmarkCard from './BookmarkCard';
import BookmarkListItem from './BookmarkListItem';
import LoadingSpinner from '../LoadingSpinner';
import { FiGrid, FiList } from 'react-icons/fi';

const BookmarkLibrary = ({ viewMode = 'grid', searchQuery = '', activeView = { type: 'all' }, className = '' }) => {
  ...
  return (
    <div className={`p-6 ${className}`} role="main" aria-label="Bookmark Library">
      {viewMode === 'grid' ? (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      <div ref={loadMoreRef} className="flex justify-center pt-8">
        {isFetchingNextPage && (
          ...
          >
            ...
            <span className="text-gray-500 dark:text-gray-400">Loading more bookmarks...</span>
          </motion.div>
        )}
