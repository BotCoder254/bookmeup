...
import { 
  FiStar, FiArchive, FiExternalLink, FiMoreVertical, 
  FiFolder, FiClock, FiArrowUp, FiArrowDown 
} from 'react-icons/fi';
import { useInfiniteBookmarks, useToggleFavorite, useToggleArchive, useVisitBookmark } from '../../hooks';
import { formatDate, extractDomain, getFaviconUrl, truncateText } from '../../utils';

const BookmarkTable = ({ searchQuery = '', activeView = { type: 'all' }, className = '' }) => {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteBookmarks(filters);
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="w-8 h-8 flex items-center justify-center">
            ...
            <div className="w-6 h-6 bg-gray-200 dark:bg-gray-700 rounded flex items-center justify-center" style={{ display: faviconUrl ? 'none' : 'flex' }}>
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center space-x-2">
            ...
            >
              <FiStar className={`w-4 h-4 ${bookmark.is_favorite ? 'fill-current' : ''}`} />
            </button>
            <button
              onClick={handleVisit}
            ...
            <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
              {table.getRowModel().rows.map((row, index) => (
                <motion.tr
                  ...
                  onClick={() => {
                    visitBookmark.mutate(row.original.id);
